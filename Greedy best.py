...
def greedy_search(start, goal=GOAL_STATE, heuristic=manhattan_distance):
    counter = itertools.count()
    open_list = [(heuristic(start, goal), next(counter), start)]
    came_from = {}
    visited = set()
    nodes_expanded = 0
    ...
    while open_list:
        ...
        for neighbor, move in get_neighbors(current):
            if neighbor not in visited:
                ...
                heapq.heappush(open_list, (heuristic(neighbor, goal), next(counter), neighbor))
